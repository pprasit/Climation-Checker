...
namespace FliSharp
{
    public class FLI : IDisposable
    {
        ...
        */
        ...
        [DllImport("libfli64.dll", CallingConvention = CallingConvention.StdCall, CharSet = CharSet.Ansi)]
        private static extern int FLIGetCameraModeString(IntPtr dev, int mode_index, [MarshalAs(UnmanagedType.LPStr)] StringBuilder mode_string, int len);
        public string GetCameraModeString(int mode_index)
        {
            StringBuilder sb = new StringBuilder(MAX_STRING_LEN);
            int status = FLIGetCameraModeString(dev, mode_index, sb, MAX_STRING_LEN);
            if (0 != status)
                return null;
            return sb.ToString();
        }
